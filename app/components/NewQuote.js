...
export default function NewQuote(props) {
  ...
  const onSave = async() => {
    let abc=[];  
    abc = await AsyncStorage.getItem('quotes');
    console.log('nietos',abc);
    ...
    await AsyncStorage.getItem('quotes', (err, quotes) => {
...
// abc.push(freq)
        // freq[author,text]=author,text;
        console.log('author :',freq);
        AsyncStorage.setItem('quotes', JSON.stringify(quotes), () => {
          if (!edit) dispatch(addQuote(freq));
          else dispatch(updateQuote(freq));
